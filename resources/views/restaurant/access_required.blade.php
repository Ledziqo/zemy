...
<section class="max-w-3xl rounded-md border border-zem-border bg-zem-card p-6">
    <x-status :status="$restaurant->dashboard_access_status ?? 'payment_required'" />
    ...
    <div class="mt-5 rounded-md border border-zem-border bg-zem-bg p-4 text-sm text-zem-muted">
        ...
        <p>Access status: <strong class="text-white">{{ str_replace('_', ' ', $restaurant->dashboard_access_status ?? 'payment required') }}</strong></p>
    </div>
