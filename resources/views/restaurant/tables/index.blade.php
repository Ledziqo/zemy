...
<form method="post" action="{{ route('restaurant.tables.store') }}" class="mb-6 grid gap-3 rounded-md border border-zem-border bg-zem-card p-4 md:grid-cols-[1fr_1fr_auto_auto]">
    ...
    <input name="table_name" placeholder="Table name optional" class="rounded-md border border-zem-border bg-zem-bg px-3 py-3">
    ...
    <button class="rounded-md bg-zem-gold px-5 py-3 font-bold text-white">Generate table QR</button>
</form>
...
@foreach($tables as $table)
    ...
    <article class="rounded-md border border-zem-border bg-zem-card p-4">
        <div class="flex items-start justify-between gap-3">
            <div>
                <p class="text-xs font-bold uppercase tracking-widest text-zem-gold">Table QR</p>
                <h2 class="mt-1 font-display text-2xl font-bold">{{ $table->table_name ?: 'Table '.$table->table_number }}</h2>
                <p class="text-sm text-zem-muted">Table number {{ $table->table_number }}</p>
            </div>
            <x-status :status="$table->is_active ? 'active' : 'cancelled'" />
        </div>

        <div class="mt-4 grid place-items-center rounded-md bg-white p-4">
            <img src="{{ route('restaurant.tables.qr', $table) }}" alt="QR code for table {{ $table->table_number }}" class="h-56 w-56">
        </div>
        ...
        <div class="mt-3 grid grid-cols-2 gap-2">
            <a href="{{ route('restaurant.tables.qr', $table) }}" download="zemtab-{{ $restaurant->slug }}-table-{{ $table->table_number }}.svg" class="rounded-md bg-zem-gold px-4 py-2 text-center text-sm font-bold text-white">Download QR</a>
        ...
        <details class="mt-4 rounded-md border border-zem-border bg-zem-bg p-3">
            <summary class="cursor-pointer text-sm font-bold">Edit table</summary>
            <form method="post" action="{{ route('restaurant.tables.update', $table) }}" class="mt-3 grid gap-3">
                ...
                <button class="rounded-md bg-zem-gold px-4 py-2 font-bold text-white">Save table</button>
            </form>
