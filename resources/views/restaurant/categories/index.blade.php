...
<div class="mb-6 rounded-md border border-zem-border bg-zem-card p-4">
    <h2 class="mb-3 font-display text-lg font-bold">Add new category</h2>
    <form method="post" action="{{ route('restaurant.categories.store') }}" class="flex flex-wrap gap-3">
        @csrf
        <input name="name" required placeholder="Category name" class="rounded-md border border-zem-border bg-zem-bg px-3 py-2">
        <input name="sort_order" type="number" placeholder="Sort order" class="rounded-md border border-zem-border bg-zem-bg px-3 py-2">
        <label class="flex items-center gap-2 rounded-md border border-zem-border bg-zem-bg px-3 py-2"><input name="is_active" type="checkbox" value="1" checked> Active</label>
        <button class="rounded-md bg-zem-gold px-4 py-2 font-bold text-white">Add category</button>
    </form>
</div>
    ...
    @forelse($categories as $category)
        <div class="rounded-md border border-zem-border bg-zem-card p-4">
            ...
            <details class="mt-3 rounded-md border border-zem-border bg-zem-bg p-3">
                <summary class="cursor-pointer text-sm font-bold text-zem-cream">Edit category</summary>
                ...
                <form method="post" action="{{ route('restaurant.categories.destroy', $category) }}" class="mt-3">@csrf @method('DELETE')<button class="rounded-md border border-red-300 bg-red-50 px-4 py-2 text-sm font-bold text-red-700 transition hover:border-red-500 hover:bg-red-100">Delete category</button></form>
            </details>
        </div>
    @empty
