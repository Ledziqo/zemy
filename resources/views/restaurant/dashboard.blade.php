@extends('layouts.dashboard', ['heading' => $restaurant->name, 'eyebrow' => $restaurant->businessTypeLabel().' '.__('Overview')])
...
<div class="mt-4 rounded-md border border-zem-gold/30 bg-zem-gold/10 p-4"><p class="text-sm text-zem-muted">{{ __('Revenue today') }}</p><p class="mt-2 text-3xl font-extrabold text-zem-gold">{{ number_format($revenue) }} ETB</p></div>
<section class="mt-6 rounded-md border border-zem-border bg-zem-card p-4">
    <div class="flex items-center justify-between"><h2 class="font-display text-xl font-bold">{{ __('Recent orders') }}</h2><a class="text-sm font-bold text-zem-gold" href="{{ route('restaurant.orders.index') }}">{{ __('Open work board') }}</a></div>
...
<section class="mt-6 rounded-md border border-zem-border bg-zem-card p-4">
    <h2 class="font-display text-xl font-bold">{{ __('Popular items this week') }}</h2>
    <div class="mt-4 grid gap-3">
