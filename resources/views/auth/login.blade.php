...
<head>
    ...
    <script>
        tailwind.config = { theme: { extend: { colors: { zem: { bg: '#F7F7F4', gold: '#E85D5D', muted: '#71717A' } } } } }
    </script>
</head>
<body class="bg-zem-bg text-[#18181B] antialiased">
<main class="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top,rgba(232,93,93,.16),transparent_35%),#F7F7F4] px-5 py-10">
    <form method="post" action="/login" class="w-full max-w-md rounded-2xl border border-[#DADAD6] bg-white p-6 shadow-2xl">
        @csrf
        <a href="/" class="inline-flex items-center">
            <img src="{{ asset('logo/zemtab-full-transparent-porcelain-coral.png') }}" alt="ZemTab" class="h-16 w-auto">
        </a>
        ...
        <div class="mt-6 space-y-4">
            ...
            <button class="w-full rounded-lg bg-zem-gold py-3 font-extrabold text-white transition hover:bg-[#B94040]">Login</button>
        </div>

        <div class="mt-5 grid grid-cols-2 gap-3">
            <button type="button" data-demo-email="admin@zemtab.test" class="rounded-lg border border-[#DADAD6] px-3 py-2 text-sm font-bold text-[#18181B] hover:border-zem-gold">Login as Admin</button>
            <button type="button" data-demo-email="owner@bolebistro.test" class="rounded-lg border border-[#DADAD6] px-3 py-2 text-sm font-bold text-[#18181B] hover:border-zem-gold">Login as Restaurant</button>
        </div>
