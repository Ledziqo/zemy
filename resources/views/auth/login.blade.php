...
<head>
    ...
    <script>
        tailwind.config = { theme: { extend: { colors: { zem: { bg: '#050505', gold: '#ef233c', muted: '#a3a3a3' } } } } }
    </script>
</head>
<body class="bg-zem-bg text-white antialiased">
<main class="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top,rgba(239,35,60,.22),transparent_35%),#050505] px-5 py-10">
    <form method="post" action="/login" class="w-full max-w-md rounded-2xl border border-white/10 bg-white/[.04] p-6 shadow-2xl">
        @csrf
        <a href="/" class="inline-flex items-center">
            <img src="{{ asset('logo/zemtab-full-transparent-dark.png') }}" alt="ZemTab" class="h-16 w-auto">
        </a>
