...
<head>
    ...
    <script>
        tailwind.config = { theme: { extend: { colors: { zem: { bg: '#F8FAFC', gold: '#FF8200', navy: '#000000', muted: '#667085' } } } } }
    </script>
</head>
<body class="bg-zem-bg text-zem-navy antialiased">
<main class="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top,rgba(255,130,0,.16),transparent_35%),#F8FAFC] px-5 py-10">
    <form method="post" action="/login" class="w-full max-w-md rounded-2xl border border-[#D8E0E7] bg-white p-6 shadow-2xl">
        @csrf
        <a href="/" class="inline-flex items-center">
            <img src="{{ asset('logo/zemtab-black-orange-pantone151-icon-text-transparent.png') }}" alt="ZemTab" class="h-16 w-auto">
        </a>
        ...
        <div class="mt-6 space-y-4">
            ...
            <button class="w-full rounded-lg bg-zem-gold py-3 font-extrabold text-white transition hover:bg-[#D66A00]">Login</button>
        </div>
