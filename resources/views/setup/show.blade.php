        ...
        <form method="post" action="/setup/run" class="mt-6">
            ...
            <div class="mb-5 grid gap-3 md:grid-cols-2">
                <label class="grid gap-1 text-sm">
                    ...
                    <input name="db_host" value="{{ old('db_host', $db['host'] ?? 'srv2081.hstgr.io') }}" placeholder="srv2081.hstgr.io" class="rounded-lg border border-[#D8E0E7] bg-white px-3 py-2">
                </label>
                <label class="grid gap-1 text-sm">
                    <span class="font-bold">DB Name</span>
                    <input name="db_database" value="{{ old('db_database', $db['database'] ?? 'u409029281_zemtab') }}" class="rounded-lg border border-[#D8E0E7] bg-white px-3 py-2">
                </label>
                <label class="grid gap-1 text-sm">
                    <span class="font-bold">DB Username</span>
                    <input name="db_username" value="{{ old('db_username', $db['username'] ?? 'u409029281_zemtab') }}" class="rounded-lg border border-[#D8E0E7] bg-white px-3 py-2">
                </label>
                <label class="grid gap-1 text-sm">
