...
@foreach($restaurants as $restaurant)
    ...
    <form method="post" action="{{ route('admin.restaurants.update', $restaurant) }}" class="grid gap-3 rounded-md border border-zem-border bg-zem-card p-4 md:grid-cols-6">
        ...
        <select name="dashboard_access_status" class="rounded-md border border-zem-border bg-zem-bg px-3 py-2">
            <option value="active" @selected($restaurant->dashboard_access_status === 'active')>Dashboard active</option>
            <option value="payment_required" @selected($restaurant->dashboard_access_status === 'payment_required')>Payment required</option>
            <option value="revoked" @selected($restaurant->dashboard_access_status === 'revoked')>Revoked</option>
        </select>
