        ...
        <div class="divide-y divide-zem-border">
            @forelse($restaurants as $restaurant)
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
                    ...
                    @php($accessStatus = !$restaurant->is_active ? 'inactive' : ($hasDashboardAccessStatus ? ($restaurant->dashboard_access_status ?? 'active') : 'active'))
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-semibold {{ $subscription?->effectiveStatus() === 'expired' ? 'text-red-300' : 'text-zem-muted' }}">Subscription: {{ $subscription ? ucfirst($subscription->effectiveStatus()) : 'No subscription' }}</span>
                        @if($accessStatus !== 'active')
                            <x-status :status="$accessStatus" />
                        @endif
                    </div>
                </div>
            @empty
